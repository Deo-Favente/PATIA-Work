...
def make_move(current_state : State, direction : Move, dimension : int) -> State | None:
    ...
    if current_state:
        ...
        if(direction == UP and x > 0): 
            new_state[x*dimension+y] = new_state[(x-1)*dimension+y]
            new_state[(x-1)*dimension+y] = 0
            return new_state

        if(direction == DOWN and x < dimension-1):
            new_state[x*dimension+y] = new_state[(x+1)*dimension+y]
            new_state[(x+1)*dimension+y] = 0
            return new_state
        
        if(direction == LEFT and y > 0): 
            new_state[x*dimension+y] = new_state[x*dimension+(y-1)]
            new_state[x*dimension+(y-1)] = 0
            return new_state
        
        if(direction == RIGHT and y < dimension-1): 
            new_state[x*dimension+y] = new_state[x*dimension+(y+1)]
            new_state[x*dimension+(y+1)] = 0
            return new_state
